...
from ...aggregates.access_token import AccessToken, TokenFamily
from ...enums import RefreshStrategy, TokenStatus
from ...interfaces.services.token.access_token_service import IAccessTokenService
...
class AccessTokenService(IAccessTokenService):
    ...
    def _analyze_refresh_security(
        ...
        if hasattr(token, 'suspicious_activity_score') and token.suspicious_activity_score > 0.5:
    ...
    def _handle_suspicious_refresh(self, token: AccessToken, warnings: list[str]) -> None:
        ...
        if not hasattr(token, 'suspicious_activity_score'):
            token.suspicious_activity_score = 0.0
    ...
    def _balance_rotation_schedule(
        ...
        for i, token in enumerate(tokens):
            if token.refresh_strategy == RefreshStrategy.ROTATE:
                ...
                if hasattr(token, 'next_rotation_at') and new_rotation_time != token.next_rotation_at:
    ...
    def _security_focused_schedule(
        ...
        for token in tokens:
            if hasattr(token, 'suspicious_activity_score') and token.suspicious_activity_score > 0.3:
                # More frequent rotation for suspicious tokens
                new_rotation_time = token.expires_at - timedelta(minutes=30)
                if hasattr(token, 'next_rotation_at') and token.next_rotation_at != new_rotation_time:
                    token.next_rotation_at = new_rotation_time
                    result["schedule_changes"] += 1
    ...
    def _performance_focused_schedule(
        ...
        for token in tokens:
            device_trust_score = getattr(token, 'device_trust_score', 0.5)
            suspicious_activity_score = getattr(token, 'suspicious_activity_score', 0.0)
            
            if device_trust_score > 0.8 and suspicious_activity_score < 0.1:
    ...
    def generate_token_analytics(
        ...
        for token in tokens:
            ...
            # Security metrics
            suspicious_score = getattr(token, 'suspicious_activity_score', 0.0)
